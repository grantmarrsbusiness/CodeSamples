...
public class PlayerController : MonoBehaviour {
	...
	void Update () {
		...
		if (Time.timeScale == 1)
		{
			...
			if (!inWater){
				...
				if (jumpPerformed){
					jumpPerformedTime += Time.deltaTime;
				}
				else {
					jumpPerformedTime = 0;
				}
	...
	void Attacks () {
		...
		if (!Input.GetKey(KeyCode.LeftControl) && !Input.GetKey(KeyCode.Joystick1Button8) || !movement.crouching.allowCrouching){
			...
			waitingBetweenAttacksTimerGround += Time.deltaTime;
			waitingBetweenAttacksTimerAir += Time.deltaTime;
	...
	void DetermineGroundedState () {
		...
		||	Physics.Linecast(pos + transform.forward*(maxGroundedRadius2*0.75f) - transform.right*(maxGroundedRadius2*0.75f) + maxGroundedHeight2, pos + transform.forward*(maxGroundedRadius2*0.75f) - transform.right*(maxGroundedRadius2*0.75f) + maxGroundedDistanceDown, out hit, noWaterCollisionLayers)){
			...
			if (!colliderInWall && (Physics.Linecast(pos + maxGroundedHeight2, pos + maxGroundedDistanceDown, out hit, noWaterCollisionLayers) || sliding || !rigidBody) && (!inMidAirFromJump || inMidAirTimer > 0.2f)){
				if (!angHit){
	...
	void PushOffWall () {
		...
		jumpedOffClimbableObjectTimer += Time.deltaTime;

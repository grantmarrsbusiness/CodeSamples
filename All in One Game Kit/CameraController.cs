...
public class CameraController : MonoBehaviour {
	...
	void OnEnable () {
		...
		yDif = 0;
		//set camera rotation
		fpOrbitRotation = Quaternion.Euler (yFpDegLerp, xFpDegLerp, 0);
	...
	void CamHeightDist () {
		...
		viewFrustum = CameraController.CalculateViewFrustum(transform.GetChild(0).GetComponent<Camera>(), ref nearClipDimensions);
	...
	public float xFpDegLerp;
	public float yFpDegLerp;
	public float xDif;
	public float yDif;
	...
	void FirstPersonModeMouseOrbiting () {
        ...
        if (inFirstPersonMode && firstPerson.mouseOrbiting.mouseOrbitInFirstPersonMode && (!lockedOn || !lockingOn.allowLockingOn) && !turningToClimb){
			...
			xFpDeg += Input.GetAxis ("Mouse X") * firstPerson.mouseOrbiting.xSpeed * 0.02f * (PlayerPrefs.GetFloat("sensitivity"));
			//invert y
			if ((PlayerPrefs.GetInt("invertY") != 0) == false)
			{
				yFpDeg -= Input.GetAxis ("Mouse Y") * firstPerson.mouseOrbiting.ySpeed * 0.02f * (PlayerPrefs.GetFloat("sensitivity"));
			}
			else
			{
				yFpDeg += Input.GetAxis ("Mouse Y") * firstPerson.mouseOrbiting.ySpeed * 0.02f * (PlayerPrefs.GetFloat("sensitivity"));
            ...
            //if the player is swimming, and his movement/rotation is locked (by the PlayerController)
                ...
                xFpDeg = Mathf.Lerp(xFpDeg, ClampAngle (xFpDeg, xFpDegMin, xFpDegMax), 6 * Time.deltaTime);
                yFpDeg = Mathf.Lerp(yFpDeg, ClampAngle (yFpDeg, yFpDegMin, yFpDegMax), 6 * Time.deltaTime);
            }
			
			xFpDegLerp = Mathf.Lerp(xFpDegLerp, xFpDeg, (6 + xDif/10) * Time.deltaTime);
			yFpDegLerp = Mathf.Lerp(yFpDegLerp, yFpDeg, (6 + yDif/10) * Time.deltaTime);
			xDif = Mathf.Abs(xFpDeg - xFpDegLerp);
			yDif = Mathf.Abs(yFpDeg - yFpDegLerp);
            //set camera rotation
            fpOrbitRotation = Quaternion.Euler (yFpDegLerp, xFpDegLerp, 0);
            transform.rotation = fpOrbitRotation;
	...
	void MouseOrbitMode () {
		...
		if ((!follow.alwaysFollow && mouseOrbit.alwaysMouseOrbit || mouseOrbitButtonPressed) && !lockedOn && !inFirstPersonMode){
			...
			//getting degrees of rotation for camera
			xDeg += Input.GetAxis ("Mouse X") * mouseOrbit.xSpeed * 0.02f * (PlayerPrefs.GetFloat("sensitivity"));
			//invert y
			if ((PlayerPrefs.GetInt("invertY") != 0) == false)
			{
				yDeg -= Input.GetAxis ("Mouse Y") * mouseOrbit.ySpeed * 0.02f * (PlayerPrefs.GetFloat("sensitivity"));
			}
			else
			{
				yDeg += Input.GetAxis ("Mouse Y") * mouseOrbit.ySpeed * 0.02f * (PlayerPrefs.GetFloat("sensitivity"));
	...
	void CompensateForWalls(Vector3 fromObject, ref Vector3 toTarget)
	{
		...
		Vector3 camPosition = transform.GetChild(0).GetComponent<Camera>().transform.position;
		transform.GetChild(0).GetComponent<Camera>().transform.position = toTarget;
		viewFrustum = CameraController.CalculateViewFrustum(transform.GetChild(0).GetComponent<Camera>(), ref nearClipDimensions);
		...
		transform.GetChild(0).GetComponent<Camera>().transform.position = camPosition;
		viewFrustum = CameraController.CalculateViewFrustum(transform.GetChild(0).GetComponent<Camera>(), ref nearClipDimensions);
